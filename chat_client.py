...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        background = background.resize((550, 400), Image.BICUBIC)
        background = ImageTk.PhotoImage(background)

        upload_image = Image.open('images/upload_ca.png')
        upload_image = upload_image.resize((25, 25), Image.BICUBIC)
    ...
    def add_photo(self):
        ...
        if self.image_path:
            ...
            user_image = user_image.resize((150, 140), Image.BICUBIC)
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        user_image = user_image.resize((40, 40), Image.BICUBIC)
        self.user_image = ImageTk.PhotoImage(user_image)

        # global background
        # background = Image.open("images/chat_bg_ca.jpg")
        # background = background.resize((1600, 1500), Image.BICUBIC)
        # background = ImageTk.PhotoImage(background)

        global group_photo
        group_photo = Image.open('images/group_ca.png')
        group_photo = group_photo.resize((60, 60), Image.BICUBIC)
        ...
        for Emoji in emoji_data:
            ...
            emojis = emojis.resize((20, 20), Image.BICUBIC)
    ...
    def clients_online(self, new_added):
        if not new_added:
            ...
            for user_id in self.clients_connected:
                ...
                user = user.resize((45, 45), Image.BICUBIC)
